...
export default function OrganizationCardsPage({ route, navigation }: Props) {
  ...
  const orgCards = useMemo(() => {
    return cards?.filter((card) => card.organization.id === orgId) || [];
  }, [cards, orgId]);

  const orgGrantCards = useMemo(() => {
    // Filter grant cards by finding the associated card's organization
    return (
      grantCards?.filter((grantCard) => {
        const associatedCard = cards?.find(
          (card) => card.id === grantCard.card_id,
        );
        return associatedCard?.organization.id === orgId;
      }) || []
    );
  }, [grantCards, cards, orgId]);

  useEffect(() => {
    const generatePatterns = async () => {
      if (!orgCards && !orgGrantCards) return;

      const allCards = [...(orgCards || []), ...(orgGrantCards || [])];
  ...
  const combineCards = useCallback(() => {
    ...
    const transformedGrantCards = orgGrantCards
      ?.map((grantCard) => ({
        ...grantCard,
    ...
    const filteredCards = orgCards?.filter(
      (card) =>
  ...
  useEffect(() => {
    ...
    if (orgCards && orgGrantCards) {
      combineCards();
    }
  }, [orgCards, orgGrantCards, combineCards]);
  ...
  useEffect(() => {
    const loadSavedOrder = async () => {
      ...
      try {
        const savedOrder = await AsyncStorage.getItem(`cardOrder-${orgId}`);
        if (savedOrder) {
  ...
  // Show empty state if no cards for this organization
  if (sortedCards && sortedCards.length === 0) {
    return (
      <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
        <Ionicons name="card-outline" color={palette.muted} size={60} />
        <Text style={{ color: palette.muted, marginTop: 16, fontSize: 16 }}>
          No cards for this organization
        </Text>
      </View>
    );
  }
      ...
      <ReorderableList
        ...
        renderItem={({ item }) => (
          <CardItem
            ...
            isActive={false}
            onPress={(_card) => {
              // For now, we'll just show the card without full navigation
              // This could be enhanced to show card details in a modal
              // or navigate to the main Cards tab with the specific card
            }}
